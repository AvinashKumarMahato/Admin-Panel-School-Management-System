...
const TeacherProfile = () => {
  const { dToken, profileData, setProfileData, getProfileData, updateProfileData } = useContext(TeacherContext);
  const { currency, backendUrl } = useContext(AppContext);
  const [isEdit, setIsEdit] = useState(false);

  // Add debug logging
  useEffect(() => {
    console.log('Current token:', dToken);
      ...
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          <div className='flex items-center gap-2 mt-1 text-gray-600'>
            <p>{profileData.degree} - {profileData.expert}</p>
          ...
          <p className='text-gray-600 font-medium mt-4'>
            Salary: <span className='text-gray-800'>{currency} {isEdit ? <input type='number' onChange={(e) => setProfileData(prev => ({ ...prev, salary: e.target.value }))} value={profileData.salary} /> : profileData.salary}</span>
          </p>
